...
export function createEmptySkillGapFilters() {
  return { evidenceLevel: "", learningStatus: "", importance: "", category: "", search: "" };
}
...
function skillGapSearchText(record) {
  const relatedJobs = Array.isArray(record?.relatedJobs) ? record.relatedJobs : [];
  return [
    record?.skill,
    record?.category,
    record?.notes,
    record?.portfolioOpportunity,
    ...relatedJobs.flatMap((job) => [job?.company, job?.role]),
  ].map(normalizedSearch).filter(Boolean).join(" ");
}

export function filterSkillGaps(records, filters = {}) {
  if (!Array.isArray(records)) return [];
  const search = normalizedSearch(filters.search);

  return records.filter((record) =>
    (!filters.evidenceLevel || record?.evidenceLevel === filters.evidenceLevel) &&
    (!filters.learningStatus || record?.learningStatus === filters.learningStatus) &&
    (!filters.importance || record?.importance === filters.importance) &&
    (!filters.category || record?.category === filters.category) &&
    (!search || skillGapSearchText(record).includes(search))
  );
}

export function keywordTrendsEmptyMessage({ analyzedJobDescriptions, trendCount, filteredCount } = {}) {
  if (analyzedJobDescriptions === 0) {
    return "Keyword analysis needs saved job descriptions. Add a description to a saved job or application to see demand trends.";
  }
  if (trendCount > 0 && filteredCount === 0) return "No keyword trends match these local filters.";
  return "No keyword trends were identified in the analyzed job descriptions.";
}

export function refreshAnnouncement({ preserveData = false, outcome = "stale" } = {}) {
  if (!preserveData) return "";
  if (outcome === "success") return "Analytics refresh complete.";
  if (outcome === "failure") return "Analytics refresh failed.";
  return "";
}

export function skillGapFrequencyLabel(frequency) {
  if (frequency === 1) return "Emerging";
  if (Number.isFinite(frequency) && frequency > 1) return "Recurring";
  return "";
}
