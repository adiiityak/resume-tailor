...
async function migrateLegacyFolder(companyDirName) {
  ...
  for (const group of groups.values()) {
    ...
    // Safe to remove originals now that they are copied into the new structure.
    for (const r of group.records) {
      try {
        await fs.unlink(r._file);
      } catch {
        /* ignore */
      }
    }
...
export async function getApplication(id, { full = false } = {}) {
  ...
  const matchReport = matchJson ? (() => { try { return JSON.parse(matchJson); } catch { return null; } })() : null;
...
export async function updateApplication(id, patch = {}) {
  ...
  const allowed = ["status", "jobUrl", "location", "role", "company", "notes", "matchScore", "resumeVariant"];
  const next = { ...metadata };
...
export async function duplicateApplication(id) {
  ...
  const { iso, date, hhmmss } = nowParts();
  const destDir = path.join(HISTORY_DIR, companySlug, date, `${roleSlug}-${hhmmss}`);
  ...
  const newId = `${companySlug}-${roleSlug}-${date}-${hhmmss}`;
  const newMeta = {
    ...metadata,
    id: newId,
    createdAt: iso,
