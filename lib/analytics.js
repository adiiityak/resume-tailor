import { listApplications, getApplication, getActivity } from "./applications.js";
...
function filterJobs(jobs, filters) {
  return jobs.filter((job) => {
    const saved = dateOnly(job.dateSaved);
...
function filteredReminders(reminders, applications, hasFilters) {
  if (!hasFilters) return reminders;
...
export async function getAnalytics(filters = {}, dependencies = defaultDependencies) {
  ...
  const [applicationList, jobList, master, achievementList, reminderList, skillGapList] = await Promise.all([
  ...
  const loaded = await loadApplications(listedApplications(applicationList), dependencies);
  ...
  const hasFilters = FILTER_KEYS.some((key) => approvedFilters[key] !== "");
  const applicationAnalytics = buildApplicationAnalytics({
    applications: filteredApplications,
    reminders: filteredReminders(reminders, filteredApplications, hasFilters),
  });
  ...
  const corruptedRecords =
    corruptionCount(applicationList, "summary") +
    loaded.corrupted +
