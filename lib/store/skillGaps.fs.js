...
async function readState() {
  ...
  try {
    ...
    if (error?.code === "ENOENT") return { rows: [], corrupted: 0, rawText: null };
    throw error;
  }
  ...
  try {
    ...
    return { rows: [], corrupted: 1, rawText: text };
  }
  if (!Array.isArray(parsed)) return { rows: [], corrupted: 1, rawText: text };
  ...
  return { rows, corrupted, rawText: text };
}

async function quarantineCorruptedState(state) {
  if (!state.corrupted || typeof state.rawText !== "string") return;
  const target = dataFile();
  const directory = path.dirname(target);
  await fs.mkdir(directory, { recursive: true });
  const quarantine = path.join(
    directory,
    `skill-gaps.corrupt-${Date.now()}-${randomUUID()}.json`
  );
  await fs.writeFile(quarantine, state.rawText, { encoding: "utf8", flag: "wx" });
}
...
export async function syncSkillGaps(input) {
  ...
  return serializeWrite(async () => {
    ...
    if (changed) {
      await quarantineCorruptedState(state);
