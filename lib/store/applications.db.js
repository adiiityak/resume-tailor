...
function toMetadata(row, files = {}) {
  ...
  return {
    ...
    submittedResumeVersion: row.submittedResumeVersion ?? extra.submittedResumeVersion ?? null,
    submittedCoverLetterVersion: row.submittedCoverLetterVersion ?? extra.submittedCoverLetterVersion ?? null,
    applicationSource: row.applicationSource || extra.applicationSource || "",
    baseProfileId: extra.baseProfileId ?? null,
  };
}
...
export async function loadAnalyticsApplications() {
  ...
  const rows = await db.select({
    ...
    companySlug: applications.companySlug,
    role: applications.role,
    roleSlug: applications.roleSlug,
    ...
    resumeVariant: applications.resumeVariant,
    submittedResumeVersion: applications.submittedResumeVersion,
    submittedCoverLetterVersion: applications.submittedCoverLetterVersion,
    mode: applications.mode,
    extra: applications.extra,
  }).from(applications).where(eq(applications.userId, userId));
  if (rows.length === 0) return { applications: [], corrupted: 0 };

  const ids = rows.map((row) => row.id);
  const [descriptionRows, eventRows] = await Promise.all([
    db.select({
      applicationId: applicationDocuments.applicationId,
      content: applicationDocuments.content,
    }).from(applicationDocuments).where(and(
      inArray(applicationDocuments.applicationId, ids),
      eq(applicationDocuments.kind, "job_description")
    )),
    db.select({
      applicationId: applicationActivity.applicationId,
      type: applicationActivity.type,
      toStatus: applicationActivity.toStatus,
      createdAt: applicationActivity.createdAt,
    }).from(applicationActivity).where(inArray(applicationActivity.applicationId, ids)),
  ]);
  const descriptions = new Map(descriptionRows.map((row) => [row.applicationId, row.content || ""]));
  const activityByApplication = new Map();
  for (const event of eventRows) {
    if (!activityByApplication.has(event.applicationId)) activityByApplication.set(event.applicationId, []);
    activityByApplication.get(event.applicationId).push({
      type: event.type,
  ...
  return {
    applications: rows.map((row) => ({
      ...
      matchScore: row.matchScore ?? null,
      resumeVariant: row.resumeVariant || "v1",
      baseProfileId: row.extra?.baseProfileId || null,
      submittedResumeVersion: row.submittedResumeVersion || null,
      submittedCoverLetterVersion: row.submittedCoverLetterVersion || null,
      mode: row.mode || "local",
      jobDescription: descriptions.get(row.id) || "",
      activity: activityByApplication.get(row.id) || [],
    })),
    corrupted: 0,
  };
}
...
export async function createApplication(input) {
  ...
  await db.insert(applications).values({
    ...
    submittedAt: dateValue(input.submittedAt),
    ...
    migrated: false,
    extra: input.baseProfileId ? { baseProfileId: input.baseProfileId } : {},
...
export async function updateApplication(id, patch = {}) {
  ...
  const columns = [
    "status", "jobUrl", "location", "role", "company", "matchScore", "resumeVariant", "priority", "workMode",
    "submittedAt", "submittedResumeVersion", "submittedCoverLetterVersion", "applicationSource",
  ];
  const set = {};
  for (const k of columns) if (k in patch && patch[k] !== undefined) set[k] = patch[k];
  if ("submittedAt" in set) set.submittedAt = dateValue(set.submittedAt);
